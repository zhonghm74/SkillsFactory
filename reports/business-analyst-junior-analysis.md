...
## 5) 证据四：客户收入集中度（直观业务证明）（Evidence D）

![Client Revenue Buckets](assets/client-revenue-buckets-2020.png)

![Client Churn Scenarios](assets/client-churn-scenarios-2020.png)
...
| 若流失 Top1 客户 | -2,035,274（-12.08%） | 单一客户流失即可造成两位数收入下滑 |
| 若流失 Top3 客户 | -4,935,915（-29.30%） | 头部小群体流失会显著冲击年度收入 |
| 若流失 Top5 客户 | -7,337,913（-43.56%） | 风险敞口接近“半壁营收” |

**Annotation:** 不使用学术指标，仅从“头部贡献金额+流失情景损失”即可直接看到：客户集中度高，且经营风险可量化。

## 6) 决策建议（So What / Now What）

1. **稳增长**：围绕已验证高贡献产品，推进交叉销售，延续交易密度提升。
2. **降集中风险**：建立客户集中度监控包（Top1/Top3/Top5 贡献、流失 Top1/Top3/Top5 情景损失），设置阈值与红黄灯。
3. **分层经营**：对头部客户做续约与流失预警，对腰尾客户做结构化拉升计划。
4. **数据治理**：在 ETL 中拦截空日期与异常值，减少分析噪声。

## 7) 可视化完整性与可访问性说明

- 图表类型与目标匹配：年度/结构对比采用 bar，趋势采用 line，风险情景采用 bar。
